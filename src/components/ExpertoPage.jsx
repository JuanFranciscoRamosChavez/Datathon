...
function ExpertoPage({ startTime, onAcusar }) {
  ...
  const ejecutarQuery = async () => {
    ...
    try {
      ...
      if (!/^select/i.test(q)) {
        throw new Error("PROTOCOL ERROR: Solo se permiten comandos de lectura 'SELECT'.");
      }
      const palabrasProhibidas = ['drop', 'delete', 'update', 'insert', 'alter', 'truncate', 'grant', 'revoke', 'create', 'replace'];
      if (palabrasProhibidas.some(word => new RegExp(`\\b${word}\\b`, 'i').test(q))) {
        throw new Error("⚠️ ALERTA DE SEGURIDAD: Comando bloqueado por firewall.");
      }
  ...
  const AcusacionForm = () => {
    const [acusacion, setAcusacion] = useState({ sospechoso: "", arma: "", lugar: "" });
    ...
    const handleConfirmar = () => {
      ...
      if (esCorrecta) {
        ...
        let usuario = { nombres: "Hacker", apellidoPaterno: "Anonimo", correo: "" };
        if (savedUser) {
        ...
        dataToSend.append("modo", "Experto"); // Marcamos modo Experto
    ...
    <div className="resultado-screen victoria">
      ...
      <div className="resultado-content">
        ...
        <p className="resultado-texto">
          El Señor Verdy ha sido expuesto gracias a tu análisis de datos.<br />
          Has demostrado habilidades de nivel experto en <Timer startTime={startTime} stopTime={tiempoFinal} />
        </p>
  ...
  const PantallaAdvertencia = () => (
    <div className="resultado-screen advertencia-mode">
      ...
      <div className="advertencia-content">
        ...
        <p style={{color: '#999', marginBottom: '2rem', fontSize:'1.5rem', fontFamily: "Verdana"}}>
          Si fallas de nuevo, serás eliminado del sistema permanentemente.
        </p>
        <button className="btn-advertencia" onClick={() => setResultadoFinal(null)}>
          REVISAR CONSULTAS
        </button>
      </div>
    </div>
  ...
  const PantallaDerrotaFinal = () => (
    <div className="resultado-screen derrota">
      ...
      <div className="resultado-content">
        ...
        <p className="resultado-texto">
          Has agotado tus intentos de consulta.<br />
          ...
          <div className="final-dramatico-content" style={{width: '100%', maxWidth:'600px', overflowY:'auto', maxHeight:'90vh'}}>
            ...
            <button 
                ...
                style={{
                    ...
                    textDecoration:'underline', cursor:'pointer', width:'100%', marginBottom:'1rem', fontFamily: 'Verdana', fontSize: '1.8rem'
                }}
            >
                ← Volver a los datos
            </button>
      ...
      <div className="guiado-layout">
        ...
        <div className={`cuaderno-sidebar ${showMobileSchema ? 'open' : ''}`}>
            ...
            <div className="sidebar-mobile-header">
                ...
                <button className="btn-minimizar" onClick={() => setShowMobileSchema(false)}>
                  ▼ CERRAR
                </button>
            </div>

            <div className="cuaderno-container custom-scroll" style={{padding: '0'}}>
                <div className="sidebar-tabs">
                    <button className={`tab-btn ${activeTab === 'mision' ? 'active' : ''}`} onClick={() => setActiveTab('mision')}>📋 PISTAS</button>
                    <button className={`tab-btn ${activeTab === 'schema' ? 'active' : ''}`} onClick={() => setActiveTab('schema')}>🗄️ TABLAS</button>
                </div>

                <div style={{padding: '1rem'}}>
                    {activeTab === 'mision' && (
                        <div className="mision-content">
                            <div className="mision-item"><h5>1. MOVIMIENTOS (Vestíbulo)</h5><p>Revisa <code>registro_seguridad</code> buscando coincidencias de hora entre Escarlata y White.</p></div>
                            <div className="mision-item"><h5>2. OBJETO PERDIDO</h5><p>Busca en <code>inventario_herramientas</code> items con estado 'desaparecido'.</p></div>
                            <div className="mision-item"><h5>3. MENSAJE CIFRADO</h5><p>Analiza <code>chat_interno</code> entre White y Black.</p></div>
                            <div className="mision-item"><h5>4. COARTADA FALSA</h5><p>Verifica entradas/salidas de Verdy en el 'Salón'.</p></div>
                            <div className="mision-item"><h5>5. INCONSISTENCIA (Cocina)</h5><p>Compara la declaración de Escarlata con sus logs reales.</p></div>
                            <div className="mision-item"><h5>6. REGISTRO OCULTO</h5><p>Busca entradas sin salida en el 'Invernadero'.</p></div>
                            <div className="mision-item"><h5>7. INVENTARIO ALTERADO</h5><p>Busca químicos ('Alucinógenos') con uso parcial.</p></div>
                            <div className="mision-item"><h5>8. BORRADOR ARREPENTIDO</h5><p>Filtra <code>chat_interno</code> por estado 'DRAFT'.</p></div>
                            <div className="mision-item"><h5>9. LLAMADA ENCUBIERTA</h5><p>Busca números de sistema (*#) marcados por Verdy.</p></div>
                            <div className="mision-item"><h5>10. DECLARACIONES CRUZADAS</h5><p>Compara declaraciones de Verdy vs Mostaza.</p></div>
                        </div>
                    )}

                    {activeTab === 'schema' && (
                        <div className="db-schema-list">
                            <div className="db-table-item" onClick={() => loadTableTemplate("registro_seguridad", "registro_id")}><h4>🛡️ registro_seguridad</h4><p>Historial de accesos</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("inventario_herramientas", "log_inventario_id")}><h4>🔧 inventario_herramientas</h4><p>Estado de objetos</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("chat_interno", "mensaje_id")}><h4>💬 chat_interno</h4><p>Mensajes y logs</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("registro_llamadas", "llamada_id")}><h4>📞 registro_llamadas</h4><p>Llamadas telefónicas</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("declaraciones_sospechosos", "declaracion_id")}><h4>📄 declaraciones</h4><p>Testimonios</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("personas", "persona_id")}><h4>👥 personas</h4><p>Lista de involucrados</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("salas", "sala_id")}><h4>📍 salas</h4><p>Mapa de la mansión</p></div>
                            <div className="db-table-item" onClick={() => loadTableTemplate("armas_herramientas", "herramienta_id")}><h4>🔪 armas_herramientas</h4><p>Catálogo de objetos</p></div>
                        </div>
                    )}
                </div>
            </div>
        ...
        <div id="pistas-container"> 
            <div className="sql-card">
                ...
                <div className="quick-bar">
                    <button onClick={() => setQuery("")} className="btn-clear">CLR</button>
                    <button onClick={() => insertTable("SELECT * FROM")}>SELECT *</button>
                    ...
                    <button onClick={() => insertTable("'Invernadero'")}>'Invernadero'</button>
                </div>
                <button className="btn-run-cyber" onClick={ejecutarQuery} disabled={loading}>
            ...
            {resultados && (
                <div className="results-card">
                    ...
                    ) : (<div style={{padding:'2rem', color:'#888'}}>No se encontraron datos.</div>)}
                </div>
            )}
            ...
            <button className="btn-acusar-expert" onClick={() => setIsSolving(true)}>
                🚨 TENGO AL CULPABLE
            </button>
        </div>
      </div>
